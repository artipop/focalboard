...
- **Desktop dev (hot reload) — `make dev-wails`**: runs `webpack --watch` (rebuilds `webapp/pack`) alongside `wails dev -tags "json1 sqlite3" -reloaddirs ../webapp/pack`, killing both on Ctrl+C. Go edits rebuild/restart via `wails dev`; frontend edits are rebuilt by webpack and the webview auto-reloads (Wails' own HMR isn't used — the frontend is server-served through `proxy.go`).
