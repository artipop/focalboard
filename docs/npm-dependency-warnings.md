...
>
> **СТАТУС: эшелоны 0, 1 и 2 (PR A) сделаны (2026-08-01).** Приложение на React 19,
> DnD на `@dnd-kit`, 0 уязвимостей, 2 deprecated. Осталось PR B — React Compiler
> и чистка мемоизации, и список TODO в конце.
...
## Что получилось по факту: PR A — React 19

Сделано одним PR-ом, потому что иначе не устанавливается: `react-beautiful-dnd`
(peer `^16.8.5 || ^17 || ^18`) и `react-dnd-scrolling` (`16.x || 17.x || 18.x`) не пускают
React 19 вовсе, а `react-day-picker@7` и `emoji-mart@3` — даже React 18. Промежуточная
остановка на 18.3, которую предполагал план, потребовала бы override-ов; от них отказались.

**Три вещи, которых не было видно снаружи:**

- **`@emoji-mart/react` не поддерживает React 19** (peer `^16.8 || ^17 || ^18`). Выкинут:
  это обёртка над vanilla-классом `Picker`, и своя занимает 30 строк.
- **FullCalendar 7 брать нельзя** — `core`/`react` уже 7.0.2, а плагины `daygrid`/`interaction`
  ещё 6.1.21. Взят 6.1.21 целиком, он пускает React 19.
- **`moduleResolution: "node"` ломал типы Lexical.** Он игнорирует `exports`, и TS брал
  `typescript-too-old.d.ts` — заглушку для TS < 5.2. Отсюда были «нет экспорта `$getRoot`».
  Переведено на `"bundler"` (правильная модель для Vite), убран мёртвый `paths`-вайлдкард
  на несуществующий `@custom_types`.

**DnD оказался дешевле, чем оценивалось при переносе в эшелон 2.** `hooks/sortable.tsx`
переписан с сохранением публичной сигнатуры до буквы — семь потребителей не тронуты, потому
что dnd-kit принимает `element` как ref, и хук по-прежнему отдаёт настоящий `RefObject`
(`tableHeader` читает `.current`, чтобы замерить колонку). Прямые `useDrop` свелись к новому
`useDropZone`; `monitor.isOver({shallow: true})` не понадобился — dnd-kit разрешает коллизию
в одну цель сам. Сайдбар: категории и доски стали `useSortable` с `index`/`group`, а логика
перестановки осталась нетронутой, потому что `Sortable` отдаёт `index`/`initialIndex`/
`group`/`initialGroup` — ровно две половины `DropResult`, против которого handlers и написаны.

Ушли шесть зависимостей и один хак: `react-beautiful-dnd`, `react-dnd`, оба бэкенда,
`react-dnd-scrolling` (автоскролл у dnd-kit встроенный) и **`Utils.isMobile()` как способ
выбрать бэкенд** — сенсоры выбираются по факту события, так что сенсорный ноутбук перестал
быть неразрешимым случаем.

**Мёртвое, найденное по дороге:** `editableDayPicker` не импортировал никто (это сняло самую
тяжёлую часть day-picker — `DayPickerInput` в v10 не существует); `setDraggedItemID`/
`setIsCategoryBeingDragged` в сайдбаре никогда не выставлялись в непустое значение, потому что
`onDragStart` у rbd не был подключён.

**Тесты — три находки, все про среду:**

- **`react-intl@10` теперь ESM-only**, а `transform` ловил только `.ts/.tsx`. Расширен на `.js`,
  `transformIgnorePatterns` сужен до списка ESM-пакетов: транспиляция всего `node_modules`
  исчерпывала память воркера.
- **jsdom не реализует `PointerEvent`**, а `@dnd-kit/dom` делает `event instanceof PointerEvent` —
  это TypeError, а не `false`. Плюс `ResizeObserver`, который dnd-kit читает при загрузке модуля,
  и `fetch`, который зовёт emoji-mart. Всё в `src/test/jsdomPolyfills.ts`.
- **`user-event@14` сделал API асинхронным.** 468 вызовов в 72 файлах, ни одного `await` — клик
  происходил после ассерта. Оставлен 13.5.0: падения сразу 82 → 41 сьюта.

Отдельно: **React 19 коммитит по выходу из `act`**, поэтому запрос к DOM внутри того же `act`,
что и `render`, ничего не находит — под React 17 legacy-рендер коммитил сразу. Несколько тестов
разделены на два `act`. И `dndKitSnapshotSerializer.ts` вычищает из снапшотов служебные a11y-атрибуты
dnd-kit: он проставляет их из эффекта, так что их наличие зависело от момента снятия снапшота,
а `aria-describedby` нёс ещё и сквозной счётчик — снапшоты флапали между прогонами.

**Итог:** 152/152 сьюта, 910 тестов зелёные, 9 помечены TODO (ниже). `check`, `pack`,
`i18n-extract`, `mac-dmg-wails` зелёные, `npm ci` не трогает лок. Чистая установка:
**0 ERESOLVE, 0 уязвимостей, 2 deprecated**.
...
- ~~`tsc` в тестах~~ — сделано, 0 ошибок (было 414). Стоит подумать, не поставить ли
  `tsc --noEmit` в `npm run check`: сейчас типы не проверяет никто, потому что сборка идёт
  esbuild-ом, а тесты — `@swc/jest`, и оба просто срезают типы. Именно поэтому ошибки и копились.
