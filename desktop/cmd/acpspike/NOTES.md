...
**`AskUserQuestion` в stream-json нерабочий.** Воспроизведено `-mode bridge`:
разрешение выдано, `tool_call` дошёл до `status=completed`, но вопросы никуда
не доставлены, и модель сама пишет «Вопросы остались без ответа — продублирую
их текстом». У инструмента нет транспорта до человека в headless-режиме,
поэтому **разрешать его хуже, чем запрещать**. Мост спавнит CLI с
`--disallowedTools AskUserQuestion`: тогда модель сразу задаёт вопросы обычным
текстом, который консоль показывает и на который можно ответить сообщением.
